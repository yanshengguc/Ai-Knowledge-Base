# 盐集 Distilled（Ai-Knowledge-Base）

**在配额、权限、长程记忆三重约束下，把正确的上下文在正确的时刻送进模型窗口。**

> 命名：盐是海水蒸馏后的结晶——RAG 做的事，就是把信息之海蒸馏成可引用的知识。

这是一个面向 Agent 的上下文供给系统：不是"把文档塞进向量库再捞出来"，而是解决**当 Agent 需要长期记住用户、被限流、且必须遵守权限边界时，上下文该怎么给**的问题。

## 它解决什么

用 Agent 处理私有知识时，真正难的从来不是"召回"，而是这四件事：

| 约束 | 为什么难 | 这里的做法 |
|---|---|---|
| **长程记忆** | 会话越久，上下文越臃肿，早期决策被挤掉 | 分层记忆 + 治理策略（写入/检索/淘汰各有规则，全部配置化） |
| **权限边界** | Agent 能调工具 = Agent 能越权取别人的数据 | 每次检索与工具调用都做归属校验（author == 当前用户） |
| **配额与限流** | 一个失控的 Agent 循环能烧掉整月预算 | Token 记账 + 每用户限流 + 每日配额三件套 |
| **可验证** | "它答得像对的"≠"它答的是对的" | 检索质量评估集 + 工具选择评估 + 24 项安全攻防，全部可复现 |

前后端同仓库：上传 PDF/Word/Markdown 资料，自动解析、切片、向量化入库，支持带引用溯源的流式 RAG 问答与工具调用 Agent（手写 ReAct 循环，非框架编排），已部署生产并通过安全攻防实测。

**演示路径建议**：上传一份 `.md` → 提问资料相关问题 → 看回答附带的切片级引用 → 开 Agent 模式看工具选择与调用时间线 → 右下角看每轮 Token 成本。在线演示暂不长期开放（2C2G 扛不住公网扫描），可用 `docker-compose.yml` 一键本地起。
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 自动化测试 | 184/184 绿 | 默认回归 152 + integration 21（真实 Redis/向量库）+ e2e 11（真实 LLM 全链路），共 50 个测试文件 |
...
- **已知取舍**：integration（9 个文件）与 e2e（8 个文件）默认跳过，因为需要外部服务与真实模型调用——这是**已知的取舍而非疏漏**，它们恰恰是唯一能回答"质量有没有下降"的测试。本地完整运行：`mvn test "-Dgroups=" ` 清空排除或用 CI（规划中）。
