# 项目证据包(8/20 建,8/29 增补;对外展示"我做过"的证据)

> 用法:被问到"怎么证明你做的是真的" → 从这份文档拿数据。
...
**生产复测**(部署后真实请求 <SERVER_IP>):B 列 A 清单→权限不足;B 读 A 文件→权限不足;`<script>alert(1)</script>T` 入库→净化为 `T`。✅
...
**公开地址**:http://<SERVER_IP>(手机可访问,移动端 viewport 适配)
...
**下午(ffbd271)**:
- **Agent 模式可视化(重点讲解新故事)**:诚实起点是"生产聊天链路此前没接 ReAct 循环"(只有 MCP/验证服务在用)→ 以显式开关接入:FunctionCallingService 带轨迹回调(步数/工具/参数/结果摘要),SSE 新增 tool 事件,前端渲染时间线;**Agent 的多次模型调用全部按 userId 记账**(否则绕过成本治理);前端另加回答复制按钮与流式光标
...
## 十、识图盲区修复(8/29 · 边界意识故事)

**发现**:自查发现识图完全没考虑,其中最危险的不是"没有能力",而是**静默失败**——扫描版 PDF(图片型,无文字层)经 PDFTextStripper 抽出空文本,切片/索引为空但状态仍可被误标;用户以为资料入库了,检索却永远命中不了。
**修复**:解析时检测空文本层,区分"有图片的扫描件"与"真空文件"并给可行动文案;`knowledge_file` 新增 error_msg 列,失败原因截断 500 落库、成功清空,FileVO 透传,前端 FAILED 悬浮展示原因。
**验证**:PdfParserTest 用 PDFBox 现场构造三类 PDF(空白/含图无字/正常文字);本地+生产各实测:上传扫描件 → FAILED + 完整提示("请使用可选中复制文字的 PDF")。开发中还逮住一个自己的 bug:error_msg 驼峰映射后写成 errMsg,自动映射找不到属性导致接口永远返回 null——DB 有值、jar 正确、接口为 null,逐层 javap 排查定位。
**讲法**:「我给自己系统做边界审视时发现:比报错更危险的是假装成功。扫描件会被静默标成功但永远检索不到——我把它改成显式失败并让失败原因可读。修复过程中还踩了一脚 error_msg/errMsg 的驼峰映射坑,DB 有值、编译正确、接口为 null,三层排查才定位。」
